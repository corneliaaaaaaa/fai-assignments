...
def estimate_hole_card_win_rate(
    nb_simulation, nb_player, hole_card, community_card=None
):
    """
    Use Monte Carlo simulations to estimate the probability of winning a hand by simulating
    multiple possible outcomes. It includes the following actions:
    - Random sample possible remaining community cards and opponent’s hole cards.
    - For each simulated game, compare my hand to the opponent’s hand and determine whether
      I lose, win or tie.
    - After running a large number of simulations, we can estimate the win rate (that is,
      the proportion of simulations in which we win).
    - Use the win rate to guide the decision-making process.
        - If the estimated win rate is high, we should raise.
        - If it's moderate, we should call.
        - If the win rate is low, we should fold.
    """
    ...
    win_count = sum(
        [
            _montecarlo_simulation(nb_player, hole_card, community_card)
