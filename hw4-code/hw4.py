...
def main():
    ...
    # plot image and save
    # h, w = compute_image_size()
    # plot: original image vs. reconstructed image
    # plot_image(img_vec, img_reconstruct_pca, (h, w), "pca")
    # plot_image(img_vec, img_reconstruct_ae, (h, w), "autoencoder")
    # plot_image(img_vec, img_reconstruct_deno_ae, (h, w), "denoising autoencoder")
    # plot: mean vector, top 4 eigenvectors
    # plot_component(pca.mean, "Image for mean vector", (h, w))
    # for i in range(4):
    #     plot_component(pca.components[:, i], f"Eigenface {i + 1}", (h, w))

    # compute mean squared error between original image and reconstructed image
    # mse_pca = compute_mse(img_vec, img_reconstruct_pca)
    # mse_ae = compute_mse(img_vec, img_reconstruct_ae)
    # mse_deno_ae = compute_mse(img_vec, img_reconstruct_deno_ae)
    # print(f"Mean squared error of PCA: {mse_pca}")
    # print(f"Mean squared error of Autoencoder: {mse_ae}")
    # print(f"Mean squared error of Denoising autoencoder: {mse_deno_ae}")
...
def plot_image(original_image, reconstructed_image, size, mode):
    """
    Plot the original image and reconstructed image side by side.
    """
    # reshape the flattened vectors of the original image and reconstructed image
    original_image = original_image.reshape(size)
    reconstructed_image = reconstructed_image.reshape(size)
    ...
    # reconstructed image
    plt.subplot(1, 2, 2)
    plt.title(f"Reconstructed Image ({mode})")
    plt.imshow(reconstructed_image, cmap="gray")
    plt.axis("off")

    # save image for the report
    SAVE_PATH = f"./original_vs_reconstructed_{mode}"
    plt.savefig(SAVE_PATH, bbox_inches="tight", pad_inches=0)

    # plt.show()


def plot_component(component, title, image_shape):
    """
    Plot image for vectors/components.
    """
    plt.figure()
    plt.imshow(component.reshape(image_shape), cmap="gray")
    plt.title(title)
    plt.axis("off")

    # save image for the report
    SAVE_PATH = f"./pca_component_{title}"
    plt.savefig(SAVE_PATH, bbox_inches="tight", pad_inches=0)
